...
const ContactSection = () => (
  <section className="py-24 px-4 border-t border-border" id="contact">
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-16">
        <h2 className="text-3xl lg:text-5xl font-bold mb-4">Contact Us</h2>
        <p className="text-muted-foreground text-lg">Visit our office or reach out — we're here to help.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {contacts.map((c) => (
          <div key={c.label} className="flex gap-4 items-start bg-console-surface border border-border rounded-2xl p-6">
            <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0">
              <c.icon className="w-5 h-5 text-primary" />
            </div>
            <div>
              <p className="text-xs font-bold tracking-wider uppercase text-muted-foreground mb-1">{c.label}</p>
              {c.href ? (
                <a href={c.href} className="text-foreground hover:text-primary transition-colors" target="_blank" rel="noopener noreferrer">{c.value}</a>
              ) : (
