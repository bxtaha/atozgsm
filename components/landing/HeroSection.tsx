...
const StatBar = ({ label, value, percentage, color }: { label: string; value: string; percentage: number; color: "mint" | "pink" }) => (
  <div className="space-y-2">
    <div className="flex justify-between text-xs font-bold tracking-wider uppercase">
      <span>{label}</span>
      <span className={color === "mint" ? "text-primary" : "text-secondary"}>{value}</span>
    </div>
    <div className="h-2 bg-background rounded-full overflow-hidden p-[1px]">
      <div
        className={`h-full rounded-full animate-bar-fill ${color === "mint" ? "bg-gradient-to-r from-secondary to-primary" : "bg-secondary"}`}
...
const HeroSection = () => (
  <div className="lg:col-span-5 bg-accent rounded-[2rem] p-6 lg:p-10 flex flex-col justify-between relative overflow-hidden">
    ...
    <div className="relative z-10">
      <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-console-surface border border-border mb-6">
        <span className="w-2 h-2 rounded-full bg-primary animate-pulse" />
        <span className="text-xs font-bold text-primary tracking-widest uppercase">Eligibility: Ages 18–25</span>
      </div>

      <h1 className="text-3xl lg:text-5xl font-bold tracking-tight leading-[1.1] mb-5 text-balance">
        Get Admission Support for China Universities — From Bangladesh
      </h1>
      <p className="text-muted-foreground text-base lg:text-lg max-w-[40ch] leading-relaxed">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        {["Real office & verified counselors", "Transparent fees", "Fast response within 24 hours"].map((t) => (
          <span key={t} className="inline-flex items-center gap-1.5 text-xs text-muted-foreground bg-console-surface px-3 py-1.5 rounded-full border border-border">
            <span className="text-primary">✓</span> {t}
          </span>
        ))}
      </div>

      <div className="mt-6 rounded-2xl overflow-hidden ring-1 ring-border">
        <img
          src={campusHero.src}
          ...
          height={1080}
          loading="eager"
          className="w-full h-40 lg:h-48 object-cover opacity-80 hover:opacity-100 transition-opacity duration-500"
        />
      </div>
    </div>
